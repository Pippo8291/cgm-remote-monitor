...
function storage (collection, ctx) {
  ...
  function createProfiles (objOrArray, fn) {
  ...
  function upsert (obj, fn) {
    ...
    api( ).update(query, obj, {upsert: true}, function complete (err, updateResults) {
      ...
      if (err) console.error('Problem upserting profile', err);
  ...
  api.create = create;
  api.createProfiles = createProfiles;
  api.save = save;
  api.remove = remove;
  api.last = last;
  api.indexedFields = ['created_at','startDate','defaultProfile'];
  return api;
}
